...
# Download - Dev
doppler secrets download -p skc-deck-api -c dev --no-file --format json | jq -r "$JQ_EXPRESSION" > .env
doppler secrets download -p skc-deck-api -c dev_docker --no-file --format json | jq -r "$JQ_EXPRESSION" > .env_docker_local

# Download - Prod
doppler secrets download -p skc-deck-api -c prod --no-file --format json | jq -r "$JQ_EXPRESSION" > .env_prod
...
doppler secrets get -p skc-deck-api -c prod SSL_CA_BUNDLE --plain  > certs/ca_bundle.crt
